...
class NeuralAccumulatorCell(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, ini):
        ...
        self.initial = ini
        self.W_hat = Parameter(torch.DoubleTensor(out_dim, in_dim))
        self.M_hat = Parameter(torch.DoubleTensor(out_dim, in_dim))
...
class NAC(nn.Module):
    ...
    def __init__(self, num_layers, in_dim, hidden_dim, out_dim, ini):
        ...
        for i in range(num_layers):
            layers.append(
                NeuralAccumulatorCell(hidden_dim if i > 0 else in_dim,
